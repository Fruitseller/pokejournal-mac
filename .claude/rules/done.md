...
- **Clean build.** `./scripts/test.sh build` exits 0 with zero errors and zero
  warnings.
- **All tests green.** `./scripts/test.sh unit` passes. For UI-affecting
  changes, `./scripts/test.sh test` (full suite) passes as well.
- **No SwiftData ghost objects.** If you delete models, fetch + delete
  individually — never rely on batch `context.delete(model:)` alone.
- **Sprites and pokemon.json are gitignored.** Never commit generated asset
  data. If the data pipeline changes, verify `scripts/fetch_pokemon_data.py`
